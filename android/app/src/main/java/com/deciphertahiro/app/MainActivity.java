...
public class MainActivity extends BridgeActivity {
    ...
    /*  ⛔ حماية من تكديس الإعلانات (سياسة AdMob: ممنوع إعلانان متتاليان).
        onResume يعمل أيضاً عند إغلاق أي إعلان ملء شاشة أو نموذج موافقة. */
    private volatile boolean fullScreenAdShowing = false;
    private volatile long lastFullScreenAdAt = 0L;
    private static final long AD_COOLDOWN_MS = 60L * 1000L;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // 📐 Android 16 (API 36) يفرض edge-to-edge بلا إمكانية إلغاء
    ...
    private volatile int sysInsetLeft = 0, sysInsetRight = 0;

    /**
     * targetSdk 35+ لا يسمح بإلغاء edge-to-edge، وAPI 36 يتجاهل
     * windowOptOutEdgeToEdgeEnforcement. بلا معالجة: المحتوى تحت شريط
     * الحالة، والبانر خلف شريط التنقّل (إعلان محجوب = مخالفة AdMob).
     * نُبطّن جذر المحتوى؛ وبما أن WebView لم يعد يمتدّ تحت الأشرطة تُرجع
     * env(safe-area-inset-*) صفراً فلا تبطين مزدوج مع الـ CSS.
     */
        ...
        try {
            ...
            ViewCompat.setOnApplyWindowInsetsListener(content, (v, windowInsets) -> {
                try {
                    ...
                    sysInsetLeft = bars.left;
                    sysInsetRight = bars.right;
    ...
    private void initConsent() {
        try {
            ...
            consentInformation.requestConsentInfoUpdate(
                    this, params,
                    () -> { fullScreenAdShowing = true; UserMessagingPlatform.loadAndShowConsentFormIfRequired(
                            this,
                            (FormError loadAndShowError) -> {
                                fullScreenAdShowing = false;
                                lastFullScreenAdAt = System.currentTimeMillis();
                    ...
                    new RewardedInterstitialAdLoadCallback() {
                        ...
                        public void onAdLoaded(@NonNull RewardedInterstitialAd ad) {
                            ad.setFullScreenContentCallback(new FullScreenContentCallback() {
                                @Override public void onAdShowedFullScreenContent() { fullScreenAdShowing = true; }
                                @Override public void onAdDismissedFullScreenContent() { fullScreenAdShowing = false; lastFullScreenAdAt = System.currentTimeMillis(); }
                                @Override public void onAdFailedToShowFullScreenContent(@NonNull AdError e) {
                                    fullScreenAdShowing = false; lastFullScreenAdAt = System.currentTimeMillis();
    ...
    private void showInterstitial() {
        // ⛔ نفس حارس التكديس: بيني بعد إعلان مكافأة = إعلانان متتاليان
        if (fullScreenAdShowing) return;
        if (System.currentTimeMillis() - lastFullScreenAdAt < AD_COOLDOWN_MS) { loadInterstitial(); return; }
    ...
    private void showAppOpenIfReady() {
        // 0) ⛔ لا تعرض إن كنّا للتوّ داخل/خارجين من إعلان آخر
        if (fullScreenAdShowing) return;
        if (System.currentTimeMillis() - lastFullScreenAdAt < AD_COOLDOWN_MS) return;
        // 1) جاهز ومحمّل
        ...
        try {
            appOpenAd.setFullScreenContentCallback(new FullScreenContentCallback() {
                @Override public void onAdShowedFullScreenContent() { appOpenShowing = true; fullScreenAdShowing = true; }
                @Override public void onAdDismissedFullScreenContent() {
                    appOpenShowing = false; fullScreenAdShowing = false; appOpenAd = null;
                    appOpenLastShown = System.currentTimeMillis();
                    lastFullScreenAdAt = appOpenLastShown;
                    loadAppOpen();
                }
                @Override public void onAdFailedToShowFullScreenContent(@NonNull AdError e) {
                    appOpenShowing = false; fullScreenAdShowing = false; appOpenAd = null;
    ...
    private void showBanner() {
        ...
        runOnUiThread(() -> {
            try {
                ...
                bannerVisible = true;
                // 📏 أبلغ الويب بالارتفاع الفعلي (50→90dp حسب الجهاز)
                try {
                    int hPx = getAdaptiveBannerSize().getHeightInPixels(this);
                    int hDp = (int) (hPx / getResources().getDisplayMetrics().density);
                    if (hDp <= 0) hDp = 50;
                    evalJs("window.__bannerH=" + hDp + ";"
                         + "if(window.onBannerHeight)window.onBannerHeight(" + hDp + ");");
                } catch (Exception ignored) {}
            } catch (Exception ignored) {}
        });
    }
    ...
    private AdSize getAdaptiveBannerSize() {
        try {
            ...
            widthPx -= (sysInsetLeft + sysInsetRight); // استثنِ مناطق النظام
    ...
    private void hideBanner() {
        runOnUiThread(() -> {
            ...
            bannerVisible = false;
            try { evalJs("window.__bannerH=0;if(window.onBannerHeight)window.onBannerHeight(0);"); } catch (Exception ignored) {}
        });
    }
